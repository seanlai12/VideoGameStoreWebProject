...
<%
	 ...
	 GameDB newDB = new GameDB();
	 Game newGame = newDB.selectGame(request.getParameter("name"));
	 int foo = newGame.getID();
%>
	<sql:query var="listGames" dataSource="${myDS}">
  SELECT * FROM Games where name = '<%=request.getParameter("name") %>'
 </sql:query>

	<sql:query var="listUsers" dataSource="${myDS}">
  SELECT * FROM Transactions where quantity > 1 AND gameid = <%=foo %>
  ORDER BY id DESC;
 </sql:query>
